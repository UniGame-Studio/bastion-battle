...
{
	public class BlaBlaSomeAttribute : Attribute { }

	public static class LerpityLerpRect
	{
		public static Rect Lerp(this Rect self, Rect target, float t)
		{
			return new Rect(Mathf.Lerp(self.x, target.x, t * 3.0f),
								 Mathf.Lerp(self.y, target.y, t * 4.0f),
								 Mathf.Lerp(self.width, target.width, t),
								 Mathf.Lerp(self.height, target.height, t * 4.0f));
		}
	}

	public class InformationValueDrawer : OdinAttributeDrawer<BlaBlaSomeAttribute>
	{
		private SirenixAnimationUtility.InterpolatedFloat t = 0.0f;

		private bool isMouseOver = false;

		protected override void Initialize()
		{
			this.t.Destination = 1.0f;
		}
		...
		protected override void DrawPropertyLayout(GUIContent label)
		{
			if (label != null && !string.IsNullOrEmpty(label.text))
			{
				...
				float labelWidth = EditorStyles.label.CalcWidth(label);

				position = position.AlignLeft(EditorGUIUtility.labelWidth + 2.0f);
				position.TakeFromLeft(labelWidth);
				position = position.Padding(4, 0);

				Rect infoRect = position.AlignLeft(20);

				var heightRequired =
					SirenixGUIStyles.MultiLineCenteredLabel
										 .CalcHeight("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
														 position.width) - SirenixGUIStyles.MultiLineCenteredLabel.lineHeight;

				if (this.isMouseOver)
				{
					GUILayoutUtility.GetRect(0, heightRequired * this.t, GUILayoutOptions.ExpandWidth());
				}
				else if (this.t > 0.0f)
				{
					GUILayoutUtility.GetRect(0, heightRequired * this.t, GUILayoutOptions.ExpandWidth());
				...
				{
					if (this.t.Time > 0.0f ? Event.current.IsMouseOver(position) : Event.current.IsMouseOver(infoRect))
					{
						this.t.Move(1.0f / 0.25f, Easing.InSine);
						this.isMouseOver = true;
					}
					else
					{
						this.t.Reverse(1.0f / 0.25f, Easing.OutSine);
						this.isMouseOver = false;
					}
				}

				Rect circleRect = infoRect.AlignCenter(19, 19);
				var tmp = circleRect;
				Rect infoTextRect = circleRect;
				var targetHeight = infoTextRect.height + heightRequired;
				infoTextRect.height += heightRequired * this.t;
				var tmp4 = 0.0f;

				if (this.t.Time > 0.0f)
				{
					tmp4 = position.width - circleRect.width;
					infoTextRect.width = Mathf.Lerp(circleRect.width, position.width - circleRect.width, this.t);
					circleRect = infoTextRect;
				}
				else
				{
					infoTextRect.width = 0.0f;
				}

				GUI.DrawTexture(circleRect, Texture2D.whiteTexture, ScaleMode.StretchToFill, true, 1.0f, new Color(38 / 255f, 38 / 255f, 38 / 255f), Vector4.zero,
									 new Vector4(15, 15, 15, 15));

				GUI.DrawTexture(tmp.Expand(2), Texture2D.whiteTexture, ScaleMode.StretchToFill, true, 1.0f, new Color(49 / 255f, 49 / 255f, 49 / 255f), 0,
									 float.MaxValue);

				GUI.DrawTexture(tmp, Texture2D.whiteTexture, ScaleMode.StretchToFill, true, 1.0f, new Color(38 / 255f, 38 / 255f, 38 / 255f), 0,
									 float.MaxValue);


				if (infoTextRect.width > infoRect.width)
				{
					var tmp2 = infoTextRect.AddX(infoRect.width).SubXMax(infoRect.width);

					GUI.BeginClip(tmp2);
					GUI.Label(tmp2.SetWidth(tmp4 - 20f).SetHeight(targetHeight).SetPosition(Vector2.zero),
								 "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
								 SirenixGUIStyles.MultiLineCenteredLabel);
					GUI.EndClip();
				}

				SdfIcons.DrawIcon(tmp.AlignMiddle(10), SdfIconType.Info, Color.white);
			}
			else { }
		}
	}
